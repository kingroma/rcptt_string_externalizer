...
public class ParameterValidation {
	 ...
	 * 가 있을경우 제거하기 위함 입니다.
	 */
	private static final String KEY_REMOVE_REGEX = ".plugins|.plugin|.Alpha|.properties|_ko_KR|_ko|properties|_en_US|_en|.nl";
	...
	public static String getValidationValue(String value){ 
		...
		if(value.contains("&")){
			if(Pattern.matches(VALUE_SHORTCUT_REGEX,value)){// 먼저 (&F) 와 같은 패턴을 검사합니다
				value = value.replace("&", "");
			}else if(Pattern.matches(VALUE_SHORTCUT_REGEX_EN, value)){
				value = value.replace("&", "");
			}
...
//		value = value.replaceAll("/","\\\\\\\\/");
	...
	private static String removeVersion(String inputString){
		...
		if (inputString.contains(".jar")) { // jar파일인 경우의 조건입니다.
			...
			int endIndex = inputString.indexOf(".jar");
			// _ko_4.6.0.v20161126060001.jar-
			// 위의 ko 와 4.6.0 등의 문자열을 버리기 위하여 진행합니다.
			
			if ((startIndex > 0) && (endIndex > 0) && (startIndex<endIndex)) {// 만약에 해당되는게 있을 경우에
			...
			if ((lastIndex >= 0) && (endIndex > 0) && (endIndex<inputString.length())) {// 만약에 해당되는게 있으면
